...
<Mermaid chart={`
flowchart TD
    A[User approves plan] --> B[PostToolUse:ExitPlanMode fires]
    B --> C[critique-plan.sh starts]
    C --> D{Test-First Check}
    D -->|Fail| E[Exit 2 + stderr]
    E --> F[Claude sees block message]
    D -->|Pass| G[Call Gemini 3 Flash]
    G --> H[Gemini critiques plan]
    H --> I[Call Codex with plan + Gemini's critique]
    I --> J[Codex adds missed points]
    J --> K[Return combined critique to Claude]
    K --> L[Claude revises plan or proceeds]
`} />
