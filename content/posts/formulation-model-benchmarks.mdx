---
title: "i ran my own llm benchmarks (52 cycles, 6 models)"
description: "instead of trusting leaderboards, i a/b tested free openrouter models on real coding tasks. here's what actually works."
date: "2026-02-17"
...
6 free models on openrouter, randomly assigned per cycle:
...
success = claude code finishes within 300 seconds, tests pass, files created. failure = timeout, crash, or incomplete.
...
| google/gemini-2.5-flash | 2 | 1 | 50.0% |
...
claude code would start building this, burn through 300 seconds of tool calls, and timeout. 64 tools used, zero files committed.
...
**over-specification kills.** claude code is smart enough to figure out details — it needs direction and a realistic scope, not a research paper.
...
# dropped: kimi (43%), qwen3.5-plus (50%), gemini-flash (50%).
...
- gemini only got 2 cycles before i pulled it. can't judge that.
- all cycles ran against the same task pool. different tasks might shuffle the rankings.
- the executor timeout (300s) punishes ambitious formulations. if i bumped it to 600s, kimi might climb.
